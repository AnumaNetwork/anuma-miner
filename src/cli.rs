...
pub struct Opt {
    ...
    /// The Anuma address for the miner reward
    pub mining_address: String,
    #[clap(short = 's', long = "anumad-address", default_value = "127.0.0.1", display_order = 1)]
    /// The IP of the anumad instance
    pub anumad_address: String,

    #[clap(long = "devfund", display_order = 6)]
    /// Mine a percentage of the blocks to the Anuma devfund [default: Off]
    ...
    /// Anumad port [default: Mainnet = 12412, Testnet = 12512]
    port: Option<u16>,
    ...
    /// Mine even when anumad says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to anumad  [default: false]
    ...
    /// Output logs in alternative format (same as anumad)
    pub altlogs: bool,
}
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        ...
        if !self.anumad_address.starts_with("grpc://") {
            IpAddr::from_str(&self.anumad_address)?;
            let port = self.port();
            self.anumad_address = format!("grpc://{}:{}", self.anumad_address, port);
        }
    ...
    fn port(&mut self) -> u16 {
        *self.port.get_or_insert(if self.testnet { 12512 } else { 12412 })
    }
