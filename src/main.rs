...
use crate::{
    cli::Opt, client::AnumadHandler, miner::MinerManager, proto::NotifyBlockAddedRequestMessage, target::Uint256,
};
...
async fn main() -> Result<(), Error> {
    ...
    while !shutdown.is_shutdown() {
        let mut client =
            AnumadHandler::connect(opt.anumad_address.clone(), opt.mining_address.clone(), opt.mine_when_not_synced)
                .await?;
        ...
        warn!("Disconnected from anumad, retrying");
    }
    Ok(())
}
